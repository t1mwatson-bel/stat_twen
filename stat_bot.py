...
def get_game_number_by_time(dt=None):
    """Расчет номера игры для обычной 21 (игры каждую минуту)"""
    if dt is None:
        dt = datetime.now()
    
    start_of_day = dt.replace(hour=0, minute=0, second=0, microsecond=0)
    minutes_passed = (dt - start_of_day).total_seconds() / 60
    game_number = int(minutes_passed) + 1
    
    return game_number

def get_next_game_time():
    """Возвращает время следующей игры (каждую минуту)"""
    now = datetime.now()
    
    next_game_time = now + timedelta(minutes=1)
            ...
            try:
                ...
                if info_elem:
                    ...
                    if match:
                        ...
                        logging.info(f"Найден стол #{current_number} в лобби")
    ...
    try:
        ...
        with lock:
            if game_number in active_tables:
                ...
                return
            if game_number in monitoring_games:
                logging.info(f"Игра #{game_number} уже в мониторинге, пропускаем")
                return
...
def monitor_loop():
    ...
    logging.info("🚀 Бот для обычной 21 запущен")
