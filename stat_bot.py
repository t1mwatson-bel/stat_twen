        ...
        while time.time() - start_time < max_duration:
            try:
                ...
                if "EPIPE" in str(e) or "pipe" in str(e).lower():
                    logging.error(f"Стол #{game_number}: ошибка соединения с браузером")
                    break
                else:
...
async def browser_manager():
    ...
    async with async_playwright() as p:
        ...
        pages = {}  # game_number -> {'page': page, 'context': context}
        ...
        while bot_running:
            try:
                ...
                # Если пора запускать новый стол и есть место
...
def main():
    """Точка входа"""
    logging.info("🚀 Бот для обычной 21 запущен (ОПТИМИЗИРОВАННАЯ ВЕРСИЯ)")
