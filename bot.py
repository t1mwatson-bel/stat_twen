...
HEADERS = {
    ...
    "Cookie":
        ...
        "tzo=3; is12h=0; "
        "referral_values=%7B%22type%22%3A%22reflinkid%22%2C"
        "%22val%22%3A%22s_50970m_355c_%22%2C"
        "%22additional%22%3A%7B%22name_tag%22%3A%22tag%22%7D%7D; "
        ...
        "_ym_d=1787827389; _ym_isad=2; _ym_visorc=b; "
        "mdd=1; "
        "_ga_7JGWL9SV66=GS2.1.s1787827388$o1$g1$t1787827414"
        "$j34$l0$h1219464045; window_width=150"
}
...
def get_active_games():
    try:
        url = (
            f"{BASE_URL}/service-api/main-live-feed/v3/"
            f"games1x2?cfView=3&count=40&fcountry=190&gr=415"
            f"&grMode=4&lng=ru&ref=7&selectedMs=10.146.1643503"
        )
...
def check_results():
    ...
    for entry in predictions:
        ...
        # =============================================================
        # ПРОВЕРЯЕМ ЦЕЛЕВУЮ ИГРУ + ВСЕ ДОГОНЫ
        # =============================================================
        for dogon in range(
            max_games_to_check
        ):
            ...
            # ---------------------------------------------------------
            # Ищем конкретную завершённую игру #N
            # ---------------------------------------------------------
            ...
            # ПРОВЕРКА ПРОГНОЗА
            #
            # Любая прогнозируемая карта,
            # встретившаяся в любой позиции,
            ...
            if found_card:
                ...
                print(
                    f"🎯 ПРОГНОЗ ЗАШЁЛ! "
                    f"{found_card} найдена "
                    f"в игре #{game_number} "
                    f"(догон {dogon})",
                    flush=True
                )
            ...
            # =========================================================
            # В ЭТОЙ ИГРЕ НЕ ЗАШЛО
            #
            # Но прогноз НЕ закрываем.
            # Проверяем следующий догон.
            # =========================================================
            print(
                f"⏭️ В игре #{game_number} "
                f"прогноз "
            ...
            # Если есть следующий догон —
            # просто переходим к нему.
            if dogon < (
                max_games_to_check - 1
            ):
                continue

            # =========================================================
            # СЛУЧАЙ 2 — НЕ ЗАШЛО ПОСЛЕ ВСЕХ ДОГОНОВ
            # =========================================================
            actual_target = None
...
def check_and_predict():
    ...
    for entry in predictions:
        ...
        # =============================================================
        # ПРОВЕРКА МАСТИ В ТЕКУЩЕЙ ИГРЕ
        # =============================================================
        predicted_suit = (
            predicted_cards[0][0][-1]
        )
