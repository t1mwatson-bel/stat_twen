...
def get_active_games():
    try:
        url = f"{BASE_URL}/service-api/main-live-feed/v3/games1x2?cfView=3&count=40&fcountry=190&gr=415&grMode=4&lng=ru&ref=7&selectedMs=10.146.2092323"
...
def monitor_active_games():
    ...
    for game in active_games:
        ...
        # ===== ДИАГНОСТИКА: ВЫВОДИМ ВСЕ КЛЮЧИ =====
        keys_list = [item.get("Key") for item in sc.get("S", []) if isinstance(item, dict)]
        print(f"🔍 {game_id} КЛЮЧИ: {keys_list}", flush=True)
        
        # ===== ДИАГНОСТИКА: ВЫВОДИМ ВСЕ ЗНАЧЕНИЯ =====
        for item in sc.get("S", []):
            if isinstance(item, dict):
                key = item.get("Key")
                val = item.get("Value")
                if val and val != "[]":
                    print(f"🔍 {game_id} [{key}] = {val}", flush=True)
        # ==========================================
